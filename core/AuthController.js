...
//         this.User = UserModel;
//         this.secret = config.jwtSecret || "santosh";
...
//             const existing = await this.User.findOne({ $or: [{ email }, { mobile }] });
...
//             console.log(user , "hii i am user ")
...
//             if (updates.password || updates.otp || updates.isVerified || updates.isBlocked) {
...
//              const decoded = jwt.verify(token, process.env.JWT_SECRET || this.secret);
...
//              console.log(user);
//              const user = await this.User.findOne( decoded._id );
...
class AuthController {
    ...
    async updateUser(req, res) {
        try {
            ...
            const updates = req.body;
            if (updates.otp || updates.isVerified || updates.isBlocked) {
                return res.status(400).json({ message: 'Cannot update restricted fields' });
            }
            if (updates.password) {
            ...
            const user = await this.User.findByIdAndUpdate(userId, updates, { new: true }).lean();
            delete user.password;
            delete user.otp;
            delete user.isBlocked;
            delete user.isVerified;
    ...
    async updateUserRegister(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) return res.status(401).json({ message: 'Unauthorized' });
    ...
    async authenticateToken(req, res, next) {
        ...
        try {
            ...
            // Find the user with the given role and token in the database
            const user = await User.findOne({ role, _id: decoded._id });
            if (!user) {
                return res.status(401).json({ message: "Invalid token or user not found." });
            }
