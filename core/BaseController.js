class BaseController {
  ...
  async get(req, res) {
    try {
      let filter = {};
      const { select, populate, query: qfields, params, pre, post } = this.config.get;
      ...
      if (req.query.search) {
        if (this.config.get.searchFields && this.config.get.searchFields.length) {
          // Partial search across multiple fields (regex OR conditions)
          filter.$or = this.config.get.searchFields.map(field => ({
            [field]: { $regex: req.query.search, $options: 'i' } // case-insensitive
          }));
        } else {
          // Default fallback: Mongo text index if available
          filter.$text = { $search: req.query.search };
        }
      }
      ...
      if (typeof pre === 'function') {
        await pre(filter, req);
      }

      // Pagination
        const { limit, skip } = this._parsePagination(req);
      // const finalSort = req.sort || sort || { createdAt: -1 };
      let query; 
      if(filter.$text){
  ...
  async getById(req, res) {
    try {
      ...
      let query = this.Model.findById(id)
        .select(this.config.getById.select);
  ...
  async updateById(req, res) {
    try {
      ...
      if (typeof pre === 'function') {
        ({ filter, updatePayload } = await pre(filter, updatePayload, req));
      }
      ...
      // Post-hook
      if (typeof post === 'function') {
        updated = await post(updated, req);
      }
  ...
  async deleteById(req, res) {
    try {
      ...
      // Pre-hook
      if (typeof pre === 'function') {
        filter = await pre(filter, req) || filter;
      }
